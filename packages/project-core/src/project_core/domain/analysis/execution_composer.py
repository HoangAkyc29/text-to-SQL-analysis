...
from project_core.domain.analysis.recipe_selector import (
    candidate_to_steps,
    select_recipe_for_subtask,
)
...
def build_execution_plan(
    ...
    *,
    ...
    for subtask in plan.subtasks:
        ...
        can_reuse = bool(
            best
            and best.compatibility_status == "compatible"
            and (chosen or reuse_score >= min_reuse_score)
            and (best.steps or best.op_chain)
            and all(step.op_id and not step.script_template for step in best.steps)
        )

        if can_reuse and best:
            for step in candidate_to_steps(best, params):
                steps_out.append(
                    ExecutionStepPlan(
...
def _generated_step_for_subtask(subtask: AnalysisSubtask, params: dict | None = None) -> RecipeStep:
    """Represent a planner gap without synthesizing executable source code."""
    return RecipeStep(
        step_id=str(uuid4()),
        name=f"catalog_plan_required_{subtask.id}",
        status="generated",
        params=params or {},
        script_template="",
    )
