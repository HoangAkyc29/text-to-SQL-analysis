...
from tool_selector.service import build_service
...
def get_service():
    ...
    if _service is None:
        ...
        _service = build_service(config, config.agents["tool-selector"])
    return _service
...
def main() -> None:
    ...
    uvicorn.run(
        "tool_selector.app:app",
        host="0.0.0.0",
        port=int(os.getenv("AGENT_HTTP_PORT", "18205")),
    )
