...
from sql_planner.service import build_service

load_project_env()

app = FastAPI(title="sql-planner")
_service = None
...
def get_service():
    ...
    if _service is None:
        ...
        _service = build_service(config, config.agents["sql-planner"])
    return _service
...
def main() -> None:
    ...
    uvicorn.run("sql_planner.app:app", host="0.0.0.0", port=int(os.getenv("AGENT_HTTP_PORT", "18202")))
