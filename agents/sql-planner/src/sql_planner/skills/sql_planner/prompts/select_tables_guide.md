...
- For historical fact tables (`STRANS` / `PMTRANS` / `STRANS_YYYYMM`), set `selected_target_dbs` to `db1` (or both when range spans cutoff). If omitted, pipeline still infers from `shard_plan`.
